...
- 测试与验证资格：唯一真源是仓库根 `current_state.json`（机器生成，`scripts/generate_current_state.py`）——collected / passing / skipped / canary / validation_timestamp 全部由实跑产生；公开 checkout 对私有资产依赖测试显式跳过。历史合同锁演进（2970→2982→2992→3018→3080）见 git 历史与 `docs/00_project/40_session_handoff.md`，不再在本文维护。
