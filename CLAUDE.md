...
- 测试：**2982 tests passing**（精确口径：**2981 passed + 1 skipped（收集 2982）**；**本地测试声明**，GitHub 无可见 CI；当前 HEAD `b464a8a`，validated_parent `157914ed`；R3 真实 staged 状态驱动 rollout 新增验收测试基线，本轮续写 offstage（+1）与结构搜索 T3 阶段一（+6）修正合同锁至 2970，作者语料采样粒度修复（+12）修正合同锁至 2982）
