...
- 测试与验证资格：唯一真源是仓库根 `current_state.json`（第四轮 attestation 协议：`scripts/run_attestation_bundle.py` 生成 bundle + `scripts/aggregate_current_state.py` 从原始 artifact 独立聚合）。历史合同锁演进见 git 历史与 `docs/00_project/40_session_handoff.md`，不在本文维护。
