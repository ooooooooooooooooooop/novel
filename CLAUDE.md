...
- 测试：**3018 tests passing**（精确口径：**3017 passed + 1 skipped（收集 3018）**；**本地测试声明**，GitHub 无可见 CI；当前 HEAD `b464a8a`，validated_parent `157914ed`；R3 真实 staged 状态驱动 rollout 新增验收测试基线，本轮续写 offstage（+1）与结构搜索 T3 阶段一（+6）修正合同锁至 2970，作者语料采样粒度修复（+12）修正合同锁至 2982，v2 出版文本代理验证器对抗性测试（+9）修正合同锁至 2992，observed-decision-author-signature 机制测试（+18）修正合同锁至 3018）
