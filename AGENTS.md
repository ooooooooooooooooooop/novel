...
- 用户级结束检查的项目接续说明见 `.codex/CONTINUITY.md`。Devin 侧对应机制为项目级 `.devin/hooks.v1.json` + `.devin/continuity.py`（SessionStart/UserPromptSubmit/Stop），状态存 `.workspace_local/task_control/devin_continuity/`。收到 hook 提供的当前 session/turn 后，在其指定文件登记本次意图；多步骤执行另填完成条件、证据和剩余事项。新用户请求须重新核对意图，普通问答仅登记 `answer`。不得为结束任务清空剩余事项、虚填证据或把执行改标为分析。
...
- 研究驱动复用公共输入、收据、缓存和验收边界，禁止从历史私有 driver 复制自带重试/放行逻辑。接入契约与离线检查见 `docs/00_project/56_research_driver_contract.md`；该契约不新增运行授权。
