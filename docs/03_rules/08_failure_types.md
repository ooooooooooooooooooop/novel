...
## 8.3 `generative_indicia`

- 定义：文本表面呈现出的 AI 生成指纹特征，不取决于功能贡献或风格配置
- 典型表现：
  - 结构 cliché（每段都以"就在这时"开头）
  - 过度平衡的措辞（对称的形容词对偶）
  - 密集修饰语堆叠（三个以上形容词连续修饰同一名词）
  - 模板化情绪爆发（按固定节奏出现的情绪高潮）
- 与 style_drift 的边界：style_drift 测量相对于配置风格的偏离；generative_indicia 测量绝对 AI 签名。两者可共存，不是同一轴。
- 与 redundancy 的边界：redundancy 测量功能重叠（该 unit 可与邻居合并）；generative_indicia 测量语言表面特征，与功能无关。
- 默认 severity：low / medium
- 默认阻断性：不阻断
- 路由偏好：优先 ReviewReminder，仅当跨多个 PlotUnit 重复或密度足以破坏风格记忆时提升为正式 ReviewIssue
