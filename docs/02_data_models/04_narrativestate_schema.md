...
这里的共享范围是故事中的当前局势；旁白向读者透露，不自动代表角色已经知情。
...
实现口径为“秘密或信息 → 知情角色 ID 列表”，与 `NarrativeState.private_information_map`
及 `to_prompt_context()` 一致，不是“角色 ID → 信息列表”。记录某角色的怀疑或信念时，
保留该认知层次，不能将其内容自动升级为已确认世界事实。
...
实现口径为系统追踪、读者尚不知且与当前局势强相关的信息。某角色不知道而读者已经
知道的内容，不能仅因角色不知就继续列为读者未知；角色知情分配由 `private_information_map`
等角色层记录表达。与 `public_information` 的观察范围不同，两字段不是简单的互补集合。
