...
## 2026-06-16 executable status

`src/boundary_control/orchestration.py` now provides the first minimal
executable `OrchestrationGateUnit`.

Current scope:

- verifies `HandoffPacket` integrity through `HandoffBoundaryUnit`
- consumes structured `NextRoute`
- consumes Review route packets produced by
  `HandoffBoundaryUnit.build_review_route()`
- preserves Review route `ReviewReminder` escalation fields in handoff
  `open_items`
- blocks route movement when Review route `ReviewReminder` open items lack a
  complete and valid escalation contract
- gates `ReviewUnit`, `ContinueUnit`, and `RewriteUnit` entry
- is exposed to saved novel workspaces through read-only `novel gate <name>`
- blocks explicitly blocking `open_items`
- blocks `ContinueUnit` entry when either handoff `open_items` or package
  `repair_control` still contains an open blocking `ReviewIssue`
- treats missing `ReviewIssue` status metadata as open for route-gate purposes
- blocks `ContinueUnit` and `RewriteUnit` movement when open `ReviewIssue`
  formal execution fields are missing, blank, invalid, or status-inconsistent
- runs no-regression validation at exit

Current non-scope:

- no DirectAPI or provider calls
- no UI or closed-loop runner
- no automatic execution after `novel gate <name>`
- no route override beyond the existing Review / Handoff contracts
- no object mutation or repair
