...
def main() -> int:
    ...
    # reader_handoffs 是 dim8a 裁决单位：复合声明使 MADE_EXPLICIT/
    # UNDER_EVIDENCED 失去原子语义（只说破半个/只缺半个）。prose 消费前
    # 运行时准入：COMPOSITE/UNCERTAIN → 拆分 → children 重过同一 gate +
    # 窄 preservation check；拆分失败则拦下（不进 prose）并记 telemetry。
    ...
    if review_response_path.exists():
        ...
        # 窄仲裁抽事实 → 确定性裁决 → 覆核/注入 detail 族 issue
        if draft_text:
        ...
        # 节奏仲裁阶段（dim6a）：事件链候选命中即触发——窄仲裁抽
        # state_delta/chain_function/compression 事实 → 确定性裁决
        if draft_text:
        ...
        # 预期仲裁阶段（dim7）：台账有开放预期才触发——due-event 哨兵
        # 抽事实（due信号/可兑现/已兑现/合法阻碍/有效更新）→ 确定性裁决；
        # reveal/flip 时查 prediction baseline（warning 仅报告）
        if draft_text:
        ...
        # 对已声明 intention 抽事实（结论是否被说破/证据是否足量呈现）
        # → 确定性三态裁决。review 不补造留白意图。
        if draft_text:
        ...
        # 对已声明 intention 抽事实（owning beat 在场？合法自陈？质感被
        # 命名？有可观察载体？）→ 确定性裁决。review 不补造白描意图。
        if draft_text:
    ...
    # 裁决冻结：intents 是计划不是事实——possibilities/dominant_prediction
    # 只能由正文逐字命中的 evidence 更新；台账在 objects 内就地更新随包落盘。
