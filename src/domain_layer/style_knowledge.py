...
TONE_STYLE_TRAITS: dict[str, list[ToneTraitEntry]] = {
    "克制": [
        {"trait": "情绪呈现", "instruction": "情绪优先靠身体反应（冷汗/攥拳/瞳孔一缩）提供推断证据；推断证据不足以让读者独立完成推断时直接 tell，不机械展开成感官" },
        {"trait": "节奏", "instruction": "叙述默认 20-30 字长句；情绪爆点用独立短句" },
        {"trait": "留白", "instruction": "顿悟优先用矛盾呈现（'可他从不记得来过'）；但人物认知状态转移本身是事件时可直接写出（'他忽然明白'并非一律删除）" },
...
#
# 纪律（Q1.3 自然修订验证 2026-09-11）：marker 命中只能提出 review
# candidate，不能指挥删除。8 章真实润色显示：本库标记的若干形式
# （结算排比、压缩多关系比喻、携带 hook 的收尾句、节拍型对白标记）
# 被按"AI味"删除后，独立盲评一致判原稿更优。删除只能来自
# "该形式无独特功能"的确认，不能来自"命中 marker"本身。
...
AI_FLAVOR_MARKERS: list[AiFlavorMarker] = [
    {
        ...
        "severity": "warning",
        "instructions": [
            "命中≠删除：先问该副词是否承担节奏/身体感/渐进性（如'一丝一丝''慢慢地'正在结算不可逆过程）",
            "确认无独立功能后再删（轻轻点了点头→点了点头）或换具体动作",
            "无明确质量增益时保留原文（等效带原则）",
        ],
    },
    {
        ...
        "severity": "warning",
        "instructions": [
            "命中≠词穷：先问重复是否构成意象回声（调用前文已建立意象、使过去与现在互相增值）；回声成立则不适用本条",
            "确属无功能重复时，同一情绪换不同感官的具象物",
            "不要因'像 AI 常用比喻'删除压缩多关系的比喻（方向/渐进/身体感/不可逆）",
        ],
    },
    {
        ...
        "severity": "warning",
        "instructions": [
            "命中≠删除：先问该句是复述已完成的推断，还是记录人物认知状态转移、预期方向或 forward hook",
            "只是复述已成立结论（'这意味着…'再说一遍）才可删；携带状态转移/hook（如'追过来的远不止…'）保留",
            "改动作或矛盾呈现顿悟，仅在确认顿悟内容已由剩余文本落地后进行",
        ],
    },
    {
        ...
        "severity": "low",
        "instructions": [
            "命中≠删除：真对比（两种感知/解释差异）保留",
            "空转强调可删或压缩；无明确收益默认保留原文",
        ],
    },
    {
        ...
        "severity": "low",
        "instructions": [
            "命中≠拆除：先问各项是否结算不同状态维度（如'没有修为/身份/宗门/退路'逐项撤走世界支柱）",
            "只有各项重复同一意义的堆叠排比才拆成具体句；结算型排比属保护清单",
            "台词内可豁免",
        ],
    },
    {
        ...
        "severity": "warning",
        "instructions": [
            "命中≠删除：先过替换测试——换成'他很难受'若几乎无损，才是占位符",
            "占位式泛化情绪标签改动作（'心中涌起怒火'→'攥紧拳头'）",
            "若该句承担身体结算功能（让损失/转变落地）则保留",
        ],
    },
    {
        ...
        "severity": "warning",
        "instructions": [
            "命中≠删除：先区分僵硬重复标签与携带节拍强度的标记（一字一句/顿了顿/声音不高承担对话节奏，不算僵硬）",
            "无信息重复标签删或用动作/换行替代；节拍标记的换写若无明确收益保留原文",
            "连续多句对白避免同一种标签",
        ],
    },
    {
    ...
    },
    {
        ...
        "severity": "low",
        "instructions": [
            "命中≠删除：段首连接词若承担逻辑转向（然而=逆转预期）则保留",
            "纯模板连接词删后直接进入事件/画面",
            "同一连接词连续出现即拆句",
        ],
    },
    {
        ...
        "severity": "low",
        "instructions": [
            "命中≠拆散：先核查枚举是否承担逐项结算功能（多个维度同时撤走/列出）",
            "纯罗列才改成有主次的推进",
        ],
    },
]

# --- 功能核查协议（marker 命中共用；与保护清单配套）---
# 删除任何表面形式之前，先回答这五个反事实问题。
FUNCTION_CHECK_QUESTIONS: list[str] = [
    "删掉后，是否有某一种读者体验或故事状态真的消失？",
    "把它换成一句泛化模板（'他很难受'/'气氛压抑'）是否几乎无损？",
    "它要贡献的事情，前一句是否已经完成？",
    "它是否改变后面至少一项：人物行动/读者预测/关系理解/损失分量/场景体验？",
    "把它拿去另一个高潮场景是否仍然成立？（越不可移植越可能承重）",
]

# --- 功能保护清单 ---
# 命中 AI 味 marker 后，若同时命中本清单中的承重情形，不得删除；
# 只能改写（功能保真压缩）或保留。
FUNCTION_PROTECTION_LIST: list[str] = [
    "结算型排比：各项结算不同状态维度（能力/身份/关系/退路），不是同义反复",
    "压缩多关系的比喻：同时编码方向、渐进性、身体感、不可逆性",
    "意象回声：调用前文已建立的意象，使过去与现在互相增值",
    "携带状态转移方向的确认性洞察：人物此刻才知道，且影响后续行动",
    "携带 forward hook 的收尾句：未兑现预期、新变数、开放承诺",
    "携带节拍强度的对白标记：一字一句/顿了顿等承担对话节奏",
]

# --- 润色处置纪律 ---
# 证据：8 章真实润色中无一次 FUNCTION_PRESERVING_COMPRESSION；
# 所有被独立判负的修订均含 FUNCTION_DELETION；
# 13 处纯风格互换无一带来稳定收益。
POLISH_DISCIPLINE: list[str] = [
    "NO_CHANGE 是合法 polish 结果：候选修改只构成风格互换且无明确质量增益时，默认保留原句",
    "删除只能来自'冗余确认'，不能来自'命中 marker'",
    "形式承重但模板感强时：优先功能保真改写（同功能更低成本），而不是删除",
    "收尾句携带 forward hook 时，hook 保留优先于'去总结腔'",
]
