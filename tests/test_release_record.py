...
# R1-R9 真实状态Rollout、强绑定TasteStack、结构化因果防线、人类盲评防伪与真实2章Staged Canary（+7）升至 2940；R0 修复闭环 2947；R3 真实状态驱动 rollout +1、WP3 资格证据包 +4、AuthorTemplate 验收 +5、Author 实例化语料作者模型测试 +4、深采样参数回归测试 +1 → 2963 collected；续写 offstage 修复（test_continuation_offstage.py,+1）与结构搜索 T3 阶段一（test_structural_search_t3_phase1.py,+6）→ 2970 collected；作者语料采样粒度修复（test_corpus_author_model.py,+12）→ 2982 collected。
EXPECTED_BASELINE = 2982
