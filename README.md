...
> **Tier 0 production-ready** — local staged CLI, operator-in-the-loop · **2982 tests passing**（精确口径：**2981 passed + 1 skipped（收集 2982）**；本地测试声明，GitHub 无可见 CI）· checkpoint tag `v0.1.2-tier0`
...
- full pytest baseline: 2982 tests passing（精确口径：2981 passed + 1 skipped（收集 2982）；本地测试声明，GitHub 无可见 CI；k3 provider 承重墙后完整回归基线，原 2803）
...
Baseline: **2982 tests passing**（精确口径：**2981 passed + 1 skipped（收集 2982）**；Windows 下测试请带 `PYTHONIOENCODING=utf-8`，本地测试声明，GitHub 无可见 CI）。
