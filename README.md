...
> **Tier 0 production-ready** — local staged CLI, operator-in-the-loop · **2952 tests passing**（本地测试声明 `2952 passed`，GitHub 无可见 CI）· checkpoint tag `v0.1.2-tier0`
...
- full pytest baseline: 2952 tests passing（本地测试声明，k3 provider 承重墙后完整回归基线，原 2803）
...
Baseline: **2952 tests passing**（Windows 下测试请带 `PYTHONIOENCODING=utf-8`）。
